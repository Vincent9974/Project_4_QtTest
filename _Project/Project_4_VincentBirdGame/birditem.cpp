...
BirdItem::BirdItem(QGraphicsScene *scene)
    ...
    ,m_IsRaise(true)
{
    scene->addItem(this); //将当前对象添加到游戏场景中
    m_scene = scene;
...
{
    return QRectF(60, FLY_BIRD_SIZE * 5,
                  FLY_BIRD_SIZE, FLY_BIRD_SIZE);
}
...
void BirdItem::flyLandfallAnimation()
{
    if(m_IsLandFall)
    {
        ...
        //每次下降一个小鸟高度
         m_flyAnimation->setEndValue(QPoint(pos().x(),pos().y() + FLY_BIRD_SIZE));
        //下降一个小鸟高度45m_flyAnimation->setEasingCurve(QEasingCurve::InQuad);
        m_flyAnimation->start();
        m_IsLandFall = false;
    }
}
