#include "remakedialog.h"
#include "ui_remakedialog.h"
#include <QFileDialog>
#include <QTimer>
#include <QMessageBox>

remakeDialog::remakeDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::remakeDialog)
{
    ui->setupUi(this);
    this->setWindowTitle("工程修复");

    connect(ui->buttonBox, &QDialogButtonBox::accepted,
            this, &remakeDialog::updateProgressBar);


}

remakeDialog::~remakeDialog()
{
    delete ui;
}

void remakeDialog::accept()
{

}

void remakeDialog::on_toolButton_clicked()
{
    QString folderPath = QFileDialog::getExistingDirectory(this,
                                                            "选择文件夹",
                                                            "",
                                                            QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    ui->lineEditFilePath->setText(folderPath);
}

void remakeDialog::updateProgressBar()
{
    //重置进度条
    ui->progressBar->setValue(0);

    //判断密码是否正确
    if(ui->lineEditPwd->text() != "123456")
    {
        QMessageBox::warning(this,"警告","密码错误");

    }
    else
    {
        //密码栏清零
        ui->lineEditPwd->clear();


        m_timer = new QTimer(this);
        int interval = 30; //定时30ms
        m_timer->start(interval);


        connect(m_timer, &QTimer::timeout,
                this, &remakeDialog::ProgressBarStart);
    }


}

void remakeDialog::ProgressBarStart()
{
    int value = ui->progressBar->value();
    int maxValue = 100;

    value += 1;

    // 如果达到最大值，停止计时器
    if (value >= maxValue) {
        ui->progressBar->setValue(maxValue);
        m_timer->stop();
    } else {
        // 更新进度条的值
        ui->progressBar->setValue(value);
    }
}
